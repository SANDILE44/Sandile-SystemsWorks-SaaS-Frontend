...
async function shareDeal() {
    ...
    const payload = {
        // Generate a random short ID for the shareId field if your controller doesn't do it
        shareId: "share_" + Math.random().toString(36).substring(2, 9), 
        type: "construction",
        inputs: getInputs(), // Sending the raw inputs
        results: {
            profit: latestData.profit,
            margin: latestData.margin,
            roi: latestData.roi,
        ...
        meta: {
            title: projectName,
            createdBy: "User" // Or get the actual username from localStorage
        },
        permissions: {
            mode: "view",
            isPublic: true
    ...
    try {
        ...
        // Debugging: If it still fails, this will print the server's reason
        if (!res.ok) {
        ...
        // Use the shareId we generated or the _id from Mongo
        ...
        alert("Check console for 400 error details.");
    }
}
  /* ================= EVENTS ================= */
  [
