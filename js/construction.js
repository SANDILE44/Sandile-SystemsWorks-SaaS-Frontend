...
async function shareDeal() {
    ...
    // 1. Create the payload for the public share entry
    const payload = {
        title: projectName,
        type: "construction",
        results: {
    ...
    try {
        // 2. Post to the share endpoint to get a unique ID
        ...
        // Get the ID returned by your backend (usually .id or ._id)
        const shareId = data.id || data._id;
        
        // 3. Construct the actual URL to your share.html page
        ...
        // 4. Trigger Native Share or Clipboard
        if (navigator.share) {
        ...
        alert("System Error: Could not generate a shareable link.");
    }
}
  /* ================= EVENTS ================= */
  [
