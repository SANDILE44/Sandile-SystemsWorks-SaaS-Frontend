(() => {
  ...
  async function runCalculation() {
    ...
    try {
      ...
      if (d.profit <= 0) {
        ...
        riskEl.textContent = 'This contract results in a loss. Costs exceed contract value.';
        adviceEl.textContent = 'DO NOT TAKE unless pricing is renegotiated.';
      } else if (d.margin < 10) {
        statusEl.textContent = '⚠ Low Margin';
        applyColor(statusEl, 'caution');
        riskEl.textContent = 'Very thin margin. Small overruns can eliminate profit.';
        adviceEl.textContent = 'Proceed only with strict cost control and risk buffer.';
      } else if (d.margin < 20) {
        statusEl.textContent = '🟡 Moderate Margin';
        applyColor(statusEl, 'caution');
        riskEl.textContent = 'Acceptable margin but limited safety buffer.';
        adviceEl.textContent = 'Monitor material and labor closely.';
      } else {
        statusEl.textContent = '✅ Strong Project';
        applyColor(statusEl, 'positive');
        riskEl.textContent = 'Healthy margin with buffer against overruns.';
      ...
      if (stepsList) {
        ...
        if (d.steps && d.steps.length) {
          d.steps.forEach((s) => {
            ...
            li.innerHTML = `<strong>${s.step}:</strong> ${s.message}`;
  ...
  $('resetBtn')?.addEventListener('click', () => {
    ...
    $('const-steps-list').innerHTML = ''; // clear steps
  });
