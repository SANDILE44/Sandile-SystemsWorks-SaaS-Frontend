...
async function shareDeal() {
    ...
    // Formatting the message for sharing
    const shareText = 
        `🏗️ Project Intelligence Report: ${projectName}\n` +
        `------------------------------------\n` +
        `💰 Net Profit: R${money(latestData.profit)}\n` +
        `📈 Margin: ${percent(latestData.margin)}\n` +
        `📊 ROI: ${percent(latestData.roi)}\n` +
        `✅ Decision: ${latestData.decision}\n` +
        `------------------------------------\n` +
        `Generated by Sandile SystemsWorks`;

    // Check if the browser supports the native share menu (best for mobile)
    if (navigator.share) {
        try {
        ...
        try {
            ...
            alert("Report summary copied to clipboard! You can now paste it in WhatsApp or Email.");
        } catch (err) {
